...
### 2.5 Colour discipline — the governing rules (2026-07-04)

*These rules govern ALL future UI work. Where an older section or mockup disagrees, §2.5 wins.*

1. **Hue belongs to actors, never to states.** Purple = the owner / actions you take; teal = the team; amber = the AI's voice. Grey, strikethrough, and weight express state (live vs settled). Never use an actor colour to express a state.
2. **The achromatic-history rule.** Anything settled — addressed, dismissed, or from a past round — renders in **greys only**. No teal, purple, or amber anywhere on a settled item. Secondary affordances on settled items (e.g. a jump-to-version link's arrow) appear on hover/keyboard focus. **One-way rule:** live things may be neutral; settled things may never wear hue — not even on hover (hover may lift the dim for readability; the greys stay).
3. **The presence exception.** The green online dot is the ONE state allowed to wear colour (the strongest learned convention in software). It must remain the only one: tiny, on the avatar edge, and green appears nowhere else as a state. *(Live since 2026-07-05 — BEHAVIOURS G9: the roster surfaces only, dot only; dimming keeps its one meaning, "silent".)*
4. **Amber belongs to the AI's voice alone** — its feed landmarks ("Claude published vN") and its identity mark in the huddler rail. It never appears on humans or as a state. The rail is *the participants*, human and AI alike: the AI's seat is always visually distinct from a person (a model logo / rounded square, never a person-coloured circle), so the rail reads as human–AI collaboration, not a crowd of teammates. *(Founder call 2026-07-04 — the AI keeps its rail seat + its click-to-filter.)*
5. **One filled-purple primary per screen.** Everything else purple is outline or text.
6. **Pills are for status or interaction, not metadata.** Type (comment / requested / flag) is a bare icon; slide numbers and positions are plain text. The requested-slide (stub) identity lives in its dashed-teal thumbnail treatment, not a pill.
7. **Colour must never be the only channel.** Identity = initials + colour; role = fill vs outline (owner = the one FILLED purple circle; collaborators = OUTLINE circles in their assigned colour); state = grey/strike/weight. Every signal must survive with colour removed.
8. **Colour budget.** A screen is roughly 90% neutral; colour is spent only on the current selection (purple), live team activity (teal), and the AI's landmarks (amber). Count badges are neutral (soft grey chip, dark text) for everyone — badge colour is a lie.
9. **Position answers scope.** Any control's placement must answer "what does this act on?": top-right corner = the user + whole-deck actions; on the slide = this slide; the rail = the participants.

**The collaborator palette** (`web/src/app/viewer/person-colors.tsx`): built in OKLCH at matched lightness so hues are evenly spread as perceived, excluding the purple family (the owner's) and the amber/orange family (the AI's). Every ink clears **≥3:1** on white as an outline ring and **≥4.5:1** as initials text. The sequence is pre-computed so every prefix is maximally spread — blue → crimson → green → magenta → azure → olive → rust → slate; near-neighbour hues only enter from seat 5, and the early seats never rely on red-vs-green separation alone. **Assignment is PER-HUDDLE by JOIN ORDER** (founder decision 2026-07-04): collaborator N in a deck's join order (server-resolved: share row / first comment, whichever is earlier) gets sequence[N]; the owner never consumes a seat. Trade-off, accepted: a person's colour may differ across decks; within one rail the first joiners are always maximally distinct. Phase-2 profiles will carry cross-deck identity.
...
- **Pills** (single-purpose, top corners; scope per §2.5 rule 9 — *(revised 2026-07-05, Slice C)*): brand pill top-left (logo + wordmark; on deck surfaces it extends with deck title + version chip). The top-right corner is **identity + deck-scope actions only**, ordered **huddle · export · Send to AI · Share · your avatar**, hierarchy carried by *weight, not colour*: **Share** is the only filled button (purple — the everyday primary action); **Send to AI** is a calm purple-outline split button; export is a bare grey icon. The avatar menu holds the user-level settings ("Pin toolbars" — the old standalone bottom gear is retired). **Slide-scope actions live ON the slide**: one purple "+" (bottom-right of the slide) opens the "This slide" menu — Add a comment · Flag for removal · Request a slide after this. The old unscoped Comments toggle is retired (the feed is the comments surface; per-slide counts live on the thumbnails and the panel).

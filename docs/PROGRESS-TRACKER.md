...
- **Verified by:** after redeploy, fired `update_deck` via the SlideHuddle MCP on a throwaway deck → **`version_published` arrived clean in PostHog** in ~16s: `{deck_id, version:3, source:"claude", addressed_count:0}`, `distinct_id` = deck owner. `deck_landing_viewed`/`feed_open_deck` snake_case proven earlier via a local interceptor + live prod bundle. **`feedback_added` + `send_to_ai_clicked` NOT yet fired live** — they need a signed-in browser session; the Claude-in-Chrome extension isn't connected and computer-use can't type into browsers, so these await either the extension or the founder running 2 clicks while CC watches PostHog.
...
- **New parking-lot entries:** throwaway test deck `8ff7f040adbd4cdf8ff2de` ("[G1 smoke test — safe to delete]") created to test version_published — delete when convenient.
- **Recommended next session:** founder fires `feedback_added` (comment from feed) + `send_to_ai_clicked` while signed in as a partner; confirm clean in PostHog; then build the §6 dashboards. P0.2 stays 🔵 until then.

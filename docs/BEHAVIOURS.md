...
- **Observation:** the AI version mark in the feed (A3) uses the producing model's real logo (`/logos/claude.svg`, `/logos/chatgpt.svg`) with a generic dark "AI" sparkle fallback if the source is unknown or the logo file is missing. `VersionSpineEvent.tsx` `AiMark`/`GenericAiMark`.

...
| Two-iframe sandbox split in viewer | The viewer renders each slide into **two** iframes with different sandboxes. The visible **display iframe** uses `sandbox="allow-scripts"` (no `allow-same-origin`) so Claude's HTML can run its layout / animation scripts inside an opaque origin — scripts can do whatever they want inside the iframe but can't reach `slidehuddleapp.vercel.app`. A hidden **measurement iframe** (positioned off-screen) uses `sandbox="allow-same-origin"` (no `allow-scripts`) so the parent can read its `contentDocument` to discover the deck's natural canvas size for aspect-ratio matching. **Critical: never combine `allow-scripts` and `allow-same-origin` on the same iframe** — that combination on user-supplied HTML is an XSS vector against `slidehuddleapp.vercel.app`. Splitting the responsibilities across two iframes is what keeps both features safe. |
